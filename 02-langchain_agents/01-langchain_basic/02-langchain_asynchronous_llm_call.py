...
async def chat(request: ChatRequest) -> ChatResponse:
    """
    Minimal async chat endpoint using LangChain with Gemini via OpenAI-compatible API.
    """
    try:
        ...
        # - llm.ainvoke(...) is the async variant of invoke
        # - we await it so the FastAPI event loop isn't blocked during the HTTP request
